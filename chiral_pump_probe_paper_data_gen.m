...
file_name = 'saved_data/CD_nomode_cplight_correction.mat'; %file name to save under
...
inc_ud_modes = true; %include underdamped modes
PP_setup = true; %take pump probe configurations
...
if PP_setup
 ...
 beam_param_set{2} = cat(3,[x;x;(x+1i*y)/sqrt(2);(x-1i*y)/sqrt(2); z;z;z ],...
                        [x;x;(x-1i*y)/sqrt(2);(x+1i*y)/sqrt(2); z;z;z ],...
                       [x;x;(x+1i*y)/sqrt(2);(x-1i*y)/sqrt(2); y;y;z ],...
                        [x;x;(x-1i*y)/sqrt(2);(x+1i*y)/sqrt(2); y;y;z ]);%,...
                        %[z;z;(x-1i*y)/sqrt(2);(x+1i*y)/sqrt(2); y;y;z ],...
                        %[z;z;(x+1i*y)/sqrt(2);(x-1i*y)/sqrt(2); y;y;z ]); 
 %beam_param_set{2} = cat(3,[x;x;(x+1i*y)/sqrt(2);(x-1i*y)/sqrt(2); z;z;z ],...
 % [y;y;(x+1i*y)/sqrt(2);(x-1i*y)/sqrt(2); z;z;z ]);     %gives the same                      
 ...
 % contributions from 2D configurations
  ...
  ma = [1,sqrt(2),0]/sqrt(3); 
  x1 = [cos(pi/3),sin(pi/3),0]; x2 = [cos(pi/3),-sin(pi/3),0];
...
%gamma_mode  = 0.005*delta_Ex ;  %/ hbar  -> 2 pi /  h
gamma_mode  = 0.015*delta_Ex;
%om_mode = delta_Ex
om_mode = delta_Ex+20
...
%lam_dru = 30 %too high, supresses modes
lam_dru = 20
%%
...
om1_rng =linspace(1.20e4,1.28e4,80);%{[],0};%  %range in cm^(-1) of {freq,time}
om3_rng = linspace(1.20e4,1.28e4,200) ;
t2_range_fs = linspace(0,1700,170);  
