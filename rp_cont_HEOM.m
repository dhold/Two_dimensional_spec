function [R1,R2,R3,R4,R5,R6] = rp_cont_HEOM(sz_params,rho_eg,V_ex_eg,V_ex_fe,...
                            V_ge_t,V_ef_t,prop_op_gg,prop_op_ee,t2_range,e1,e2,...
...
out_trunc_gg = tot_vib^2*H_trunc;
out_trunc_ee = N^2*tot_vib^2*H_trunc;
...
%space vector
 ...
 cj_eg = HEOM_conj({HL,N*tot_vib,tot_vib});
 cj_fe = HEOM_conj({HL,N*(N-1)/2*tot_vib,N*tot_vib});
...
for j1 = 1:N  %first transition to exciton j and then either conjugate or j->f
    ...
    tmp_gg = mtimesx(  V_ex_eg(:,:,j1),'C',rho_eg(:,:,:,:,j1));
    tmp_gg = reshape( tmp_gg, [tot_vib^2*HL,t1L]);
    tmp_ee = mtimesx(  rho_eg(:,:,:,:,j1),V_ex_eg(:,:,j1),'C');
    tmp_ee = reshape( tmp_ee, [N^2*tot_vib^2*HL,t1L]);
   ...
   % hold on
    for lp = 1:t1L %could avoid nexting other loops within this
       ...
       % plot(t2_range,tmp_ee_t([1,4],:))
        ...
        for j3 = 1:N
            if calc_rp
           % R1tmp = V_eg_right{j3}*tmp_ee_t; %stimulated emission
           % R4tmp = V_eg_left{j3}*tmp_gg_t; %-as abs but negative occ
           % implicitly (hole) so this - cancels
          R1tmp = HEOM_mult(V_ex_eg(:,:,j3),'n',tmp_ee_t,'n','R',...
                        [N*tot_vib,N*tot_vib,HL]);
          R4tmp = HEOM_mult(V_ex_eg(:,:,j3),'n',tmp_gg_t,'n','L',...
                        [tot_vib,tot_vib,HL]); 
            end
             if calc_nr
           % R2tmp = V_eg_right{j3}*conj(tmp_ee_t(cj_ee,:)); %SE
           % R3tmp = -V_eg_left{j3}*conj(tmp_gg_t(cj_gg,:)); %-as abs
          R2tmp = HEOM_mult(V_ex_eg(:,:,j3),'n',tmp_ee_t,'C','R',...
                        [N*tot_vib,N*tot_vib,HL]); %SE
          R3tmp = HEOM_mult(V_ex_eg(:,:,j3),'n',tmp_gg_t,'C','L',...
                        [tot_vib,tot_vib,HL]);  %-as abs 
             end
            ...
            R_1 = mtimesx( V_ge_sec,'T',R1tmp); %t3_range by t2_range
            R_4 = mtimesx( V_ge_sec,'T',R4tmp);
            ...
            R_2 =  mtimesx(V_ge_sec,'T',R2tmp); %t3_range by t2_range
            R_3 =  mtimesx(V_ge_sec,'T',R3tmp);
