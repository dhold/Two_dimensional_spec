function [R1,R2,R3,R4,R5,R6,rho_gg_t,rho_ee_t] = rp_cont_HEOM5(sz_params,...
            rho_eg,rho_ge,V_ge,V_ef,prop_op_gg,prop_op_ee,t2_range,e1,e2,...
      calc_nr,calc_rp,av1_g,av1_f,av2_g,av2_f,H_trunc,...
        om_0, ww ,pop_only,tol,t_verbose) ;
 % Version 5 allows for a Gaussian filter to select specific frequencies of
 % beating during the population time (prop to om_0 = omega_1 - omega_2 etc) 
 %with the filter width "ww" associated with the pulse duration
...
if ~exist('ww','var'); filter_t2 = false; 
else; filter_t2 = true;  filter_fun = @(w) exp(-ww^2*(w-om_0).^2);
end
...
if t_verbose
    ...
    for lp = 1:t1L %could avoid nexting other loops within this
        ...
        tmp_ee_sec = tmp_ee(:,lp);
            if filter_t2 % Gaussian filter
        tmp_gg_t = (OD_wrapper2(t2_range,rho_gg_prop,tmp_gg_sec,filter_fun,[],out_trunc_gg,'ode45',tol)).';  
        tmp_ee_t = (OD_wrapper2(t2_range,rho_ee_prop,tmp_ee_sec,filter_fun,[],out_trunc_ee,'ode45',tol)).'; 
            else
        ...
        if calc_rp
        tmp_gg_sec2 = tmp_gg2(:,lp);  tmp_ee_sec2 = tmp_ee2(:,lp);
            if filter_t2 % Gaussian filter
        tmp_gg_cnj = (OD_wrapper2(t2_range,rho_gg_prop,tmp_gg_sec,filter_fun,[],out_trunc_gg,'ode45',tol)).';  
        tmp_ee_cnj = (OD_wrapper2(t2_range,rho_ee_prop,tmp_ee_sec,filter_fun,[],out_trunc_ee,'ode45',tol)).'; 
            else
