...
b_range = pi*linspace(0,1/64,300);
clear minimum2 fval2
...
f = matlabFunction(tmp);
ff = @(x) f(x(1),x(2),x(3),x(4));
    if lp > 1; guess = minimum2{lp-1};
    else; guess = pi/4*[-1,-1,3,-3]; end
[minimum2{lp},fval2{lp}] =  fminunc(ff ,guess);
end
...
rnj = cat(1,fval2{:})<2e-6; %good points;
...
for lp2= 1:4
   ...
   angle_fn2(:,lp2) = minimum{1}(lp2) + tmp.m.*b_range.^2;
...
figure; plot([b_range(1),b_range(4:end)],[0,min_smooth(4:end)])
%%
