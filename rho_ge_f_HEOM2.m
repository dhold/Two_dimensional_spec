...
if ~exist('tol','var')
    ...
    rho_ge_ft = zeros(N*HL,length(om_1_rng),N);
...
rho_0_lio(j)=1;    %initially in equilibirum
 ...
 sup_op_ge_freq = (1i*speye(N*HL)*om_1_rng(lp) - sup_op_ge); 
 if nargout==2
 tic
 end
 rho_ge_ft(:,lp,j) = sup_op_ge_freq\rho_0_lio; %solves LA system
 ...
 if nargout==2%this is currently just a test to see the comparitive rate 
     ...
     %I will change the code to use this but it doesn't seem to for the
     %range I am using
 if lp>1 
     
      t_meth1 = t_meth1 + toc; tic        
     x0 = rho_ge_ft(:,lp-1,j); %use last calculated as initial guess
    [x,flag] = bicg(sup_op_ge_freq,rho_0_lio,tol,100,[],[],x0); %interative solver
     t_meth2 =  t_meth2 + toc;
     if flag ~=0 % checks if an error occurs
         [lp,flag]
     end
     meth_diff(lp,j)= norm(x - rho_ge_ft(:,lp-1,j));
