function [R1,R2,R3,R4,R5,R6] = rp_cont_HEOM(sz_params,rho_eg,V_ex_eg,V_ex_fe,...
                            ...
                            calc_rp,calc_nr,av1_g,av1_f,av2_g,av2_f) ;
...
for j1 = 1:N  %first transition to exciton j and then either conjugate or j->f
    ...
    for lp = 1:t1L
        ...
        for j3 = 1:N
            ...
            R_1 = mtimesx(mtimesx( V_ge_sec,R1tmp),ori_fct_rf); %t2_range by t3_range
            ...
            R_2 =  mtimesx(mtimesx(V_ge_sec,R2tmp),ori_fct_nr); %t2_range by t3_range
            ...
            if calc_nrf
             ...
             ori_tmp1 = reshape(av1_f(j1,j2,N*(N-1)/2*j3+f,N*(N-1)/2*j4+f,:),[1,1,num_sets(1)]);
            %next the one which depends on |k_j|
            tmp2 = squeeze(av2_f(j1,j2,N*(N-1)/2*j3+f,N*(N-1)/2*j4+f,:,:));
            ori_tmp2 = reshape(e1(j1)*s_rp(1).*tmp2(:,1)+e1(j2)*s_rp(2).*tmp2(:,2)+...
                        (e2(f)-e1(j3))*s_rp(3).*tmp2(:,3),[1,1,num_sets(2)]);
            ori_fct_rp = cat(3,ori_tmp1,ori_tmp2);                      
                   
                R_6 = R_6+mtimesx(mtimesx( V_ef_t(:,:,j4,f),R6tmp),ori_fct_rp); 
            ...
            ori_fct_nr = cat(3,ori_tmp1,ori_tmp2);       
                   ...
                   R_5 = R_5+mtimesx(mtimesx( V_ef_t(:,:,j4,f),R5tmp),ori_fct_nr);       
                    end                    
                end
