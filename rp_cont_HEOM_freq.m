function [R1,R2,R3,R4,R5,R6,rho_gg_t,rho_ee_t] = rp_cont_HEOM_freq(sz_params,...
            rho_eg,rho_ge,V_ge,V_ef,prop_op_gg,prop_op_ee,w2_range,e1,e2,...
      calc_nr,calc_rp,av1_g,av1_f,av2_g,av2_f,pop_only,t_verbose) ;
...
%
% This version works in frequency space for the population time
...
t1L = size(rho_eg,2);  w2L = length(w2_range);  t3L = size(V_ge,1);
...
if ~calc_nr && ~calc_rp
 ...
 %preallocate rephasing
        R1 = complex(zeros(t3L,t1L,sum(num_sets),w2L)); %flip order at the end
        R4 = R1; R5 = R4;
    else
        R1=[]; R4=[]; R5=[];
    end
    if calc_rp
    R2 = complex(zeros(t3L,t1L,sum(num_sets),w2L));  %preallocate nonrephasing
        R3 = R2; R6 = R3;
    else
        R2 = []; R3=[]; R6 = [];
    end    
...
for lp = 1:w2L %(easily parallelised for loop)

     sup_op_gg_freq = (-(1i*speye(N*HL)*w2_rng(lp) + prop_op_gg))^(-1);  
     sup_op_ee_freq = (-(1i*speye(N^2*(N-1)/2*HL)*w2_rng(lp) + prop_op_ee))^(-1);
    if t_verbose;  tic; end
    
for j1 = 1:N  %first transition to exciton j and then either conjugate or j->f
     %2nd dipole interaction
    if pop_only;  j2_rng = j1;
   ...
   % NR-> rho_ee = rho__eg V_ge, rho_gg = V_ge rho__eg        
    ...
    tmp_ee2 = V_eg_right{j2}*rho_ge(:,:,j1); 
        ...
        tmp_gg_f = mtimesx(sup_op_gg_freq,tmp_gg); %note this is a hole
        tmp_ee_f = mtimesx(sup_op_ee_freq,tmp_ee);
        
        end
        
        if calc_rp
        tmp_gg_cnj = mtimesx(sup_op_gg_freq,tmp_gg2); %note this is a hole
        tmp_ee_cnj = mtimesx(sup_op_ee_freq,tmp_ee2);
        end
        ...
        if nargout > 6 && calc_rp && calc_nr
             rho_gg_t(:,lp,j1,j2)   = tmp_gg_f(1,:) + tmp_gg_cnj(1,:);
             rho_ee_t(:,:,lp,j1,j2) = tmp_ee_f(1:4,:) + tmp_ee_cnj(1:4,:);
        end 
        
        for j3 = 1:N
            if calc_nr

          R1tmp = V_eg_ee_left{j3}*tmp_ee_f;
          R4tmp = V_eg_gg_right{j3}*tmp_gg_f;
         ...
         if pop_only ;f4_rng = f3;  
                    ...
                    for f4 = f4_rng
                    if calc_nr
                        R6tmp = -(V_fe_right{j3,f3}*tmp_ee_f); 
...
if calc_nr  
    R3 = permute(R3,[1,4,2,3]); R2 = permute(R2,[1,4,2,3]); 
    R6 = permute(R6,[1,4,2,3]);
end
if calc_rp
    R4 = permute(R4,[1,4,2,3]); R1 = permute(R1,[1,4,2,3]);  
    R5 = permute(R5,[1,4,2,3]); 
end
