function [R1,R2,R3,R4,R5,R6,rho_gg_t,rho_ee_t] = rp_cont_HEOM3(sz_params,...
            rho_eg,V_ge,V_ef,prop_op_gg,prop_op_ee,t2_range,e1,e2,...
      calc_nr,calc_rp,av1_g,av1_f,av2_g,av2_f,H_trunc,tol) ;
...
%{
%     tmp_ee_test =  reshape(rho_eg(:,:,:,j1),N,1,HL,t1L); %proper set of Aux den mat
%     V_ge_tmp = zeros(1,N); V_ge_tmp(j2) =1;
%     tmp_ee_test = mtimesx(tmp_ee_test,V_ge_tmp); %multiply all tiers by this
%     tmp_ee_test = reshape(tmp_ee_test,[N^2*HL,t1L]);     
%     test = tmp_ee_test-tmp_ee;
%     sum(abs(test(:)))
 %}    
    %block comment contains test that this pics the correct elements
    tic
   ...
   % hold on
    for lp = 1:t1L %could avoid nexting other loops within this
        ...
        if nargout > 6
             rho_gg_t(:,lp,j1,j2)   = tmp_gg_t(1,:);
             rho_ee_t(:,:,lp,j1,j2) = tmp_ee_t(1:4,:);
        end 
       ...
       % plot(t2_range,tmp_ee_t([1,4],:))
        ...
        for j3 = 1:N
            if calc_nr
           % R1tmp = %stimulated emission  rho_ee * V_eg_
          ...
          %{
            if lp==1 
               V_eg_tmp = zeros(N,1); V_eg_tmp(j3)=1;
               tmppp = reshape(tmp_ee_t,[N,N,HL,t2L]);
               tmppp = mtimesx(tmppp,  V_eg_tmp);
               test = reshape(tmppp,[N*HL,t2L]);
               test2 = test-R1tmp ; sum(abs(test2(:)))
            end
            %}
          ...
          R3tmp = sparse(R3tmp);
            ...
            R6tmp2 = mtimesx( V_ef(:,:,j4,f),R6tmp);
           ...
           R5tmp2 = mtimesx( V_ef(:,:,j4,f),R5tmp);
